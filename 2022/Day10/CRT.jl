...
function part2(data)
    ...
    for line in data
        if line[1] == 'n' # noop
            cycle += 1
            push!(crt, (abs(register - mod(cycle, 40)) < 2) ? '#' : '.')
        else # addx
            val = split(strip(line))[2]
            cycle += 1
            push!(crt, (abs(register - mod(cycle, 40)) < 2) ? '#' : '.')
        ...
        #println("$(line), $(cycle), $(register) $(crt[end])")
    end
    println("after $(cycle) cycles the output looks like")
    for i in 1:40:240
        @printf("Cycle %3d -> %s  <- Cycle %3d\n", i, join(crt[i:i+39]), i + 39)
    end
    return false
end
...
function main()
    ...
    println("testing")
    part2(readlines(open("test.txt")))
    println("executing")
    part2(readlines(open("input.txt")))
