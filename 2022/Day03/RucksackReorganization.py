...
def priority(item):
  # a =1 .... z=26
  score = ord(item.lower()) - ord('a')+1
  # A = 26  ... Z=52
  if item.isupper():
    score += 26
  return score

def get_sack_score(rucksack):
...
def get_group_badge(e1,e2,e3):
  # turn the rucksack into a set of chars and strip any \n
  elf1 = set(e1.strip())
  elf2 = set(e2.strip())
  elf3 = set(e3.strip())
...
def take_three(iterable):
  for i in range(0,len(iterable), 3):
    yield iterable[i:i+3]

def part2(data):
  score = 0
  for e1,e2,e3 in take_three(data):
    common = get_group_badge(e1,e2,e3)
    score += priority(common.pop())
  return score
