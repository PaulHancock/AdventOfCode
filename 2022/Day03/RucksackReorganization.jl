...
function priority(item)
    # score a=1 .. z=26
    score = convert(Int, lowercase(item)) - convert(Int, 'a') + 1
    # upper case is lower +26
    score += convert(Int, item) < convert(Int, 'a') ? 26 : 0
    return score
end

function get_sack_score(rucksack)
    middle = length(rucksack) ÷ 2
    left = @view rucksack[begin:middle]
    right = @view rucksack[middle+1:end]
    common = left ∩ right
    score = priority(common[1])
    return score
end
...
function part1(data)
    score = 0
    for line in data
        score += get_sack_score(strip(line))
    end
    return score
end
...
function main()
    @assert part1(readlines(open("test1.txt"))) == 157
    ...
    @assert part2(readlines(open("test1.txt"))) == 70
    open("input1.txt") do f
        # read till end of file
        data = readlines(f)
